...
public class GameBoard {
	...
	/*
	 * public int[] getPlayArea() { int minY = 10; int maxY = -1; int minX = 10; int
	 * maxX = -1;
	 * 
	 * 
	 * for (int x = 0; x < MAX_WIDTH; x++) { for (int y = 0; y < MAX_HEIGHT; y++) {
	 * if (spaces[x][y].getSType() != LandType.EMPTY) { // if space is occupied if
	 * (x < minX) { minX = x; } if (x > maxX) { maxX = x; } if (y < minY) { minY =
	 * y; } if (y > maxY) { maxY = y; } } } }
	 * 
	 * int newMinX = maxX - 4; int newMaxX = minX + 4; int newMinY = maxY - 4; int
	 * newMaxY = minY + 4;
	 * 
	...
	public ArrayList<ArrayList<Space>> getGameBoardScoreGroups() {
		return scoredSpaces;
	}
	
	public int getMaxSpaceGroupSize() {
		int maxLength = 0;
